...
export function CoinModal({ open, side, onOpenChange }: CoinModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        className={`relative z-10 bg-card border border-border rounded-2xl p-6 w-[320px] max-w-[90vw] space-y-5 ${side === "a" ? "rotate-180" : ""}`}
      >
        {/* Header */}
        <div className="flex items-center justify-between">
          <span className="text-foreground font-bold text-lg">🪙 Coins</span>
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <button
            ...
            className="w-9 h-9 rounded-xl bg-muted border border-border flex items-center justify-center text-foreground hover:opacity-80 transition-colors"
          >
                ...
                <div
                  ...
                  className={`w-16 h-16 rounded-full flex items-center justify-center text-xl font-black shadow-lg shrink-0 ${flipping ? "coin-flipping" : ""
                      ...
                      : isHeads
                        ...
                        : "bg-linear-to-br from-gray-400 to-gray-600 text-foreground"
                    }`}
        ...
        {/* Results summary */}
        {results && (
          <div className="text-center text-base bg-muted rounded-xl py-2.5 px-4">
            <span className="text-green-400 font-semibold">{heads} Heads</span>
            {" · "}
            <span className="text-primary font-semibold">{tails} Tails</span>
          </div>
        )}
